...
- **When an incident is open:** the whole display alternates between the layout above (~2 s) and a full-width incident view (~4.65 s) — the incident name renders in a fixed-width font at the top (≤15 chars: 1 line static; ≤30 chars: 2 lines wrapped static; >30 chars: a 2-row "wrapping marquee" where row 1 shows the current 15-char window and row 2 shows the next 15-char window, both scrolling in lockstep so the text flows row 1 → row 2 → loop); the centered colored impact word in the middle; and a colored band at the bottom with age + affected-component count. The big severity tile is dropped on the incident frame since the impact word is already colored by the same scheme; the full 64 px of width lets the title breathe.
